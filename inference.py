...
def main():
    ...
    if full_frames_RGB is None or crop is None or quad is None:
        raise ValueError("Crop function returned None")
        pass
    ...
    if not os.path.isfile('temp/' + base_name + '_coeffs.npy') or args.exp_img is not None or args.re_preprocess:
        ...
        base_name_cleaned = os.path.basename(os.path.splitext(base_name)[0])
        output_path = os.path.join('temp', base_name_cleaned + '_coeffs.npy')
        np.save(output_path, semantic_npy)
    ...
    stabilized_file = os.path.join(temp_dir, f"{base_name}_stabilized.npy")

    if not os.path.isfile(stabilized_file) or args.re_preprocess:
        imgs = []
        for idx in tqdm(range(len(frames_pil)), desc="[Step 3] Stabilize Expression In Video:"):
            source_img = trans_image(frames_pil[0 if args.one_shot else idx]).unsqueeze(0).to(device)

            if args.one_shot:
                semantic_source_numpy = semantic_npy[0:1]
            else:
                if idx >= len(semantic_npy):
                    print(f"Warning: Index {idx} out of bounds for semantic_npy with shape {semantic_npy.shape}. Skipping frame.")
                    continue
                semantic_source_numpy = semantic_npy[idx:idx + 1]

            if semantic_source_numpy.size == 0:
                print(f"Error: semantic_source_numpy is empty at index {idx}. Skipping frame.")
                continue
            ...
            try:
                ...
                img_stabilized = np.uint8(((output['fake_image'].squeeze(0).permute(1, 2, 0).cpu().clamp_(-1, 1).numpy() + 1) / 2) * 255)
                imgs.append(cv2.cvtColor(img_stabilized, cv2.COLOR_RGB2BGR))
            except Exception as e:
                print(f"Error processing frame {idx}: {e}")
                continue

        np.save(stabilized_file, imgs)
        del D_Net
    else:
        print('[Step 3] Using saved stabilized video.')
        imgs = np.load(stabilized_file)
    ...
    for idx in tqdm(range(len(imgs)), desc='[Step 5] Reference Enhancement'):
        img = imgs[idx]
        try:
            pred, _, mask_sharp = enhancer.process(img, img, face_enhance=True, possion_blending=False)
            if mask_sharp is None:
                mask_sharp = np.zeros_like(img)
            imgs_enhanced.append(pred)
        except Exception as e:
            print(f"[Step 5] Error enhancing frame {idx}: {e}")
    ...
    output_path = f'temp/{args.tmp_dir}/result.mp4'
    ...
    def get_available_memory(device='cuda'):
        if device == 'cuda' and torch.cuda.is_available():
            ...
            gpu_mem_cached = torch.cuda.memory_reserved(0)
            gpu_mem_available = gpu_mem_total - (gpu_mem_allocated + gpu_mem_cached)
            return gpu_mem_available
        elif device == 'cpu':
            return psutil.virtual_memory().available
        else:
            return 0

    def calculate_optimal_batch_size(model_memory_estimate, img_size, device='cuda'):
        available_memory = get_available_memory(device)
        img_memory = img_size[0] * img_size[1] * 3 * 4
        per_image_memory = model_memory_estimate + img_memory
        safety_margin = 0.8
        max_images = int((available_memory * safety_margin) // per_image_memory)
        return max(max_images, 1)

    MODEL_MEMORY_ESTIMATE = 200 * 1024 * 1024
    IMG_SIZE = (384, 384)
    batch_size = calculate_optimal_batch_size(MODEL_MEMORY_ESTIMATE, IMG_SIZE, device='cuda')
        ...
        try:
            ...
            with torch.no_grad():
                try:
                    ...
                    print(f"[Step 6] Model inference failed for frame {i}: {e}")
                    continue

                if args.up_face in ['sad', 'angry', 'surprise']:
                    tar_aus = exp_aus_dict.get(args.up_face, None)
                else:
                    tar_aus = None
                ...
                if args.up_face == 'original':
                    ...
                    try:
                        test_batch = {
                            ...
                            'tar_aus': tar_aus.repeat(len(incomplete), 1) if tar_aus is not None else None
                        }
                        ...
                        print(f"[Step 6] Expression modification failed for frame {i}: {e}")
                        cur_gen_faces = img_original
            ...
            for p, f, xf, c in zip(pred, frames, f_frames, coords):
                try:
                    ...
                    restored_img, ff, full_mask = [
                        cv2.resize(x, (512, 512)) for x in (restored_img, ff, np.float32(np.zeros_like(ff[:, :, 0])))
                    ]
                    ...
                    print(f"[Step 6] Post-processing failed for frame {i}: {e}")
                    continue
        except Exception as e:
            print(f"[Step 6] Error synthesizing frame {i}: {e}")
            continue
    ...
    command = f'ffmpeg -loglevel error -y -i {args.audio} -i {output_path} -strict -2 -q:v 1 {args.outfile}'
...
def datagen(frames, mels, full_frames, frames_pil, cox):
    ...
    try:
        ...
        base_name = current_directory

    refs = []
    image_size = 256

    print(f"[Debug] Total frames: {len(full_frames)}")
    print(f"[Debug] Example frame shape: {full_frames[0].shape if full_frames else 'No frames available'}")
    print("[Info] Running face detection...")
    face_det_results = face_detect(full_frames, args, jaw_correction=True)

    if not face_det_results or all(fd is None for fd in face_det_results):
        print("[Warning] No faces detected. Ensure video contains visible faces or check detection settings.")
        return

    print(f"[Info] Faces detected: {len(face_det_results)}")

    available_memory = virtual_memory().available
    safe_batch_size = min(args.LNet_batch_size, available_memory // (image_size ** 2 * 3 * 4))

    kp_extractor = KeypointExtractor()
    try:
        fr_pil = [Image.fromarray(frame) for frame in frames]
        lms = kp_extractor.extract_keypoint(fr_pil, f'temp/{base_name}_landmarks.txt')
    ...
    for inverse_transform, crop, full_frame, face_det in zip(inverse_transforms, crops, full_frames, face_det_results):
        if face_det is None:
            print("[Warning] Skipping frame due to failed face detection.")
            continue
